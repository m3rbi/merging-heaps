 ...
 *
 * This function takes a pointer to a linked list and constructs a min heap from its elements.
 * The new heap is constructed in-place, meaning that the original linked list is modified.
 *
 * @param list A pointer to the linked list.
 * @param is_sorted A boolean flag indicating whether the original list is sorted.
 *                  If true, a more efficient build algorithm is used.
 ...
 *
 * This function removes and returns the minimum element from the given min heap.
 * The min heap is modified accordingly to maintain its heap property.
 *
 * @param heap The min heap from which to extract the minimum element.
 * @return A pointer to the minimum element in the min heap, or NULL if the heap is empty.
 */
 ...
 *
 * This function takes two min heaps, `heap_a` and `heap_b`, and merges them into a single min heap.
 * The resulting min heap will contain all the elements from both `heap_a` and `heap_b`.
 * The original min heaps are modified accordingly, heap `heap_b` will be empty after the merge,
 * and heap `heap_a` will contain the merged elements.
 * NOTE: the original list of `heap_a` is invalidated, and freed.
 *
