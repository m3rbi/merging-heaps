...
/**
 * Performs the min heapify operation on the given heap starting from the specified index.
 *
...
/**
 * Decreases the key value of a node in a min heap, and modifies the heap accordingly.
 *
 ...
 *
 * This function takes two unsorted min heaps, `heap_a` and `heap_b`, and merges them into a single heap.
 * The resulting heap will contain all the elements from both `heap_a` and `heap_b`, and will be sorted in ascending order.
 *
...
void union_min_heaps(min_heap_t *heap_a, min_heap_t *heap_b) {
    ...
    return heap_a->is_sorted && heap_b->is_sorted ? union_sorted_min_heaps(heap_a, heap_b) : union_unsorted_min_heaps(heap_a, heap_b);
}
